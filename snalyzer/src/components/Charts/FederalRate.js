...
const FederalRate = () => {
  const [data, setData] = useState([40219,25328]);
  const svgRef = useRef();
  ...
  useEffect(() => {
    ...
    const xScale = d3.scaleBand()
      ...
      .range([0, 300])
      .padding(0.5);
      const sent = ['National Parties','Regional Parties']
    const yScale = d3.scaleLinear()
      .domain([0, d3.max(data)])
      .range([150,0]);

    const colorScale = d3.scaleLinear()
      .domain([10000, 20000, 60000])
